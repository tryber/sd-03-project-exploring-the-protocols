...
const server = net.createServer((socket) => {
  socket.on('data', (data) => {
    ...
    const cpusPrint = () => {
      ...
      str.push(`Cores quantity ${cpus.length}`);

      cpus.forEach(({ model, speed }) =>
        str.push(`Model: ${model}, Speed: ${speed}`));

      return str.join('\r\n');
    };
    // solução conjunta de Rhian Castro, Luis Eduardo e Hebert Freitas, SD-03
    getLocationInfos(clientIP, (locationData) => {
      ...
      socket.write('<title>Trybe :rocket:</title></head><body>');
      socket.write(
        '<H1>Explorando os Protocolos :face_with_monocle::mag_right:</H1>',
      );
      socket.write(`<H3 data-testid="ip">ip: ${clientIP}</H3>`);
      socket.write(`<H3 data-testid="city">city: ${locationData.city}</H3>`);
      socket.write(
        `<H3 data-testid="postal_code">postal_code: ${locationData.postal_code}</H3>`,
      );
      socket.write(
